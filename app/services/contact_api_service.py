...
class ContactApiService:
    ...
    def list_contacts(self, status: Optional[str] = None) -> list[sqlite3.Row]:
        """取得聯絡人清單，支援狀態篩選。"""
        return self.contact_repo.list_all(status=status)

    def get_contact(self, contact_id: int) -> Optional[sqlite3.Row]:
        """依 ID 取得單一聯絡人詳細資訊。"""
    ...
    def update_contact(
        ...
        existing = self.contact_repo.get_by_id(contact_id)
        if not existing:
            return None
